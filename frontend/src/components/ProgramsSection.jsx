...
import { Search, ChevronRight, ChevronLeft, Award, Wifi, BookOpen, Users, MessageCircle, Eye, Star, Sparkles, Percent, CreditCard, Loader2, Heart } from 'lucide-react';
...
const ProgramsSection = ({ onOpenAuth }) => {
  ...
  const bannerSlides = [
    {
      ...
      title: 'Bourses CSC 2025',
      subtitle: 'Postulez maintenant pour les bourses du gouvernement chinois'
    },
    {
      id: 2,
      image: 'https://images.unsplash.com/photo-1541339907198-e08756dedf3f?w=800',
      title: 'Études en France',
      subtitle: 'Découvrez les programmes Campus France et Bourse Eiffel'
    },
    {
      id: 3,
      image: 'https://images.unsplash.com/photo-1562774053-701939374585?w=800',
      title: 'MBBS en Chine',
      subtitle: 'Programmes de médecine enseignés en anglais'
    },
    {
      id: 4,
  ...
  const filterOffers = (offers, query, category, filter) => {
    ...
    if (filter !== 'all') {
      switch (filter) {
        case 'new':
          results = results.filter(o => o.isNew);
          break;
        case 'fullScholarship':
          results = results.filter(o => o.hasScholarship && !o.isPartialScholarship);
          break;
        case 'partialScholarship':
          results = results.filter(o => o.isPartialScholarship);
          break;
        case 'selfFinanced':
          results = results.filter(o => o.isSelfFinanced);
          break;
        case 'online':
          results = results.filter(o => o.isOnline);
          break;
        default:
          break;
      }
  ...
  // Filter offers when search/category/filter changes
  useEffect(() => {
  ...
  // Auto-slide (only when not showing all results)
  useEffect(() => {
  ...
  const formatPrice = (offer) => {
    ...
    if (price === 0) {
      return <span className="text-green-600 font-bold">Gratuit</span>;
    }
  ...
  const getFilterTitle = () => {
    if (activeCategory) {
      return categories.find(c => c.id === activeCategory)?.name || 'Programmes';
    }
  ...
  // Display offers based on mode
  const displayOffers = showAllResults ? filteredOffers : filteredOffers.slice(0, 6);

  const suggestedPrograms = allOffers.slice(0, 4).map(offer => ({
    id: offer.id,
    title: offer.title,
    category: offer.categoryLabel || offer.category
  }));
        ...
        <form onSubmit={handleSearch} className="bg-white rounded-xl shadow-sm p-4 mb-6">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Rechercher un programme, une université, une spécialité..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:border-[#1a56db] focus:ring-2 focus:ring-[#1a56db]/20 text-sm"
              />
            </div>
            <button 
              type="submit"
              className="bg-[#1a56db] hover:bg-[#1648b8] text-white px-8 py-3 rounded-lg font-medium transition-colors flex items-center justify-center gap-2"
            >
              <Search size={18} />
              Rechercher
            </button>
        ...
        <div className="flex gap-6">
          ...
          <div className="hidden lg:block w-[240px] flex-shrink-0">
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              <div className="bg-[#1a56db] text-white px-4 py-3 font-semibold flex items-center gap-2">
                <BookOpen size={18} />
                Programmes
              </div>
            ...
            <div className="bg-gradient-to-br from-[#1a56db] to-[#1e3a8a] rounded-xl p-5 mt-4 text-white">
              ...
              <button className="w-full bg-white text-[#1a56db] py-2 rounded-lg font-medium text-sm hover:bg-blue-50 transition-colors flex items-center justify-center gap-2">
          ...
          <div className="flex-1">
            {/* Image Carousel Banner - Hidden when showing all results */}
            {!showAllResults && (
              <div className="relative rounded-xl overflow-hidden mb-6 h-[280px] group">
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-gradient-to-r from-black/60 via-black/30 to-transparent">
                      <div className="absolute bottom-0 left-0 p-6">
                        ...
                        <button className="mt-4 bg-[#1a56db] hover:bg-[#1648b8] text-white px-6 py-2 rounded-lg font-medium transition-colors">
                          En savoir plus
                        </button>
                      </div>
                    </div>
                ...
                <button 
                  onClick={prevSlide}
                  className="absolute left-3 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/40 text-white p-2 rounded-full opacity-0 group-hover:opacity-100 transition-all"
                >
                  <ChevronLeft size={24} />
                </button>
                <button 
                  onClick={nextSlide}
                  className="absolute right-3 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/40 text-white p-2 rounded-full opacity-0 group-hover:opacity-100 transition-all"
                >
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                {showAllResults && (
                  <button 
                    ...
                    className="text-[#1a56db] hover:text-[#1648b8] flex items-center gap-1 text-sm"
                  >
                    <ChevronLeft size={16} />
                    Retour
                  </button>
              ...
              {!showAllResults && filteredOffers.length > 6 && (
                <button 
                  ...
                  className="text-[#1a56db] hover:text-[#1648b8] font-medium text-sm flex items-center gap-1"
                >
                  Voir tout
                ...
                {displayOffers.map((offer) => (
                  ...
                  >
                    <div className="relative h-36 overflow-hidden">
                      <img
                        src={offer.image || 'https://images.unsplash.com/photo-1523050854058-8df90110c9f1?w=600'}
                        alt={offer.title}
                        ...
                        {(offer.badges || []).slice(0, 2).map((badge, index) => (
                          <span 
                            ...
                            className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                              badge.includes('Complète') ? 'bg-green-500 text-white' :
                              badge.includes('Partielle') ? 'bg-blue-500 text-white' :
                              badge.includes('Auto') ? 'bg-purple-500 text-white' :
                              badge.includes('Ligne') ? 'bg-teal-500 text-white' :
                              badge.includes('Populaire') || badge.includes('Top') || badge.includes('CSC') ? 'bg-orange-500 text-white' :
                      ...
                      <div className="absolute top-2 right-2 bg-white/90 backdrop-blur-sm px-2 py-0.5 rounded text-xs font-medium">
                        {offer.isOnline ? '💻 En ligne' : `${offer.country === 'Chine' ? '🇨🇳' : '🇫🇷'} ${offer.country || ''}`}
                      </div>
                    </div>
                    <div className="p-4">
                      <h4 className="font-semibold text-gray-900 text-sm mb-1 group-hover:text-[#1a56db] transition-colors line-clamp-2">
                        {offer.title}
                      </h4>
            ...
            {/* View More Button - Only when not showing all */}
            {!showAllResults && filteredOffers.length > 6 && (
              <div className="text-center mt-6">
                <button 
                  onClick={handleViewAll}
                  className="bg-[#1a56db] hover:bg-[#1648b8] text-white px-8 py-3 rounded-lg font-medium transition-colors"
                >
                  Voir tout ({filteredOffers.length} programmes)
                </button>
              </div>
            )}
          </div>

          {/* Right Sidebar - Hidden when showing all results for more space */}
          {!showAllResults && (
                ...
                <div className="space-y-3">
                  {suggestedPrograms.map((program) => (
                    <a 
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        const offer = allOffers.find(o => o.id === program.id);
                        if (offer) handleOfferClick(offer);
                      }}
                      className="flex items-start gap-3 p-2 rounded-lg hover:bg-gray-50 transition-colors group"
                    >
                      <div className="w-12 h-12 bg-[#1a56db] rounded-lg flex items-center justify-center flex-shrink-0">
                        <span className="text-white text-xs font-bold">WC</span>
                      </div>
                      <div>
                        <h5 className="text-sm font-medium text-gray-900 group-hover:text-[#1a56db] transition-colors">
                          {program.title}
                        </h5>
